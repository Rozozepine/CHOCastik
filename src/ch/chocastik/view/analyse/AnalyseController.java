...
public class AnalyseController {
    	...
    	if(mainApp.getThreadAnalyseFlag()) {
    		...
    		if(mainApp.getAnalyseEndFlag()) {
    			if(!mainApp.getMobileData().isEmpty()) {
        			...
        			messageErreur("Pas de sÈlection", "Pas de mobile sÈlectionnÈ", "Veuillez sÈlectionner un mobile sur la table.");
        		}
    		}else {
    			messageErreur("Analyse en cours", "Analyse en cours", "Une analyse est deja en cours");
    		}
	...
	private void handleNewMobile() {
		...
		if(image != null) {
			...
			messageErreur("Aucune image disponible", "Aucune image disponible", "Aucune image disponible, veuillez dÈmarrer la capture");
		}
	...
	public void dsetCameraChoice(int cam) throws Exception {
		...
		grabber.setFrameRate(60); // fr√©quence de trame
